...
def combine(exons, cds, target, log):
    ...
    for line in exons:
        ...
        if gene in cds_set:
            ...
            target.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format(gene, cds_fields[1], exon_fields[1], cds_fields[2], exon_fields[2], cds_fields[3], exon_fields[3], cds_fields[4]))
            ...
            target.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format(gene, 0, exon_fields[1], 0, exon_fields[2], 0, exon_fields[3], exon_fields[4]))
            stats['notfound'] += 1
